...
# Usage: ./schedule_with_note.sh <minutes> "<note>"
...
nohup bash -c "sleep $SECONDS && tmux send-keys -t tmux-orc:0 'Time for orchestrator check! cat /Users/jasonedward/Coding/Tmux\ orchestrator/next_check_note.txt && python3 claude_control.py status detailed' && sleep 1 && tmux send-keys -t tmux-orc:0 Enter" > /dev/null 2>&1 &
